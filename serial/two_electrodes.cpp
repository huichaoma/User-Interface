#include "two_electrodes.h"
#include "ui_two_electrodes.h"

two_electrodes::two_electrodes(QWidget *parent) :
    QMainWindow(parent),
    ui(new Ui::two_electrodes)
{
    ui->setupUi(this);
    this->setAutoFillBackground(true);
    statusBar()->setSizeGripEnabled(false); //设置是否显示右边的大小控制点
    per1 = new QLabel("二电极测量电阻率", this);
    ...
    connect(open_window,&QAction::triggered,this,&two_electrodes::open_window);
    connect(save,&QAction::triggered,this,&two_electrodes::save);
    connect(quit,&QAction::triggered,this,&two_electrodes::quit);
    ...
    connect(new_row,&QAction::triggered,this,&two_electrodes::new_row);
    connect(new_col,&QAction::triggered,this,&two_electrodes::new_col);
    connect(delete_row,&QAction::triggered,this,&two_electrodes::delete_row);
    connect(delete_col,&QAction::triggered,this,&two_electrodes::delete_col);
    connect(clear,&QAction::triggered,this,&two_electrodes::clear_data);
    QMenu *formula= menuBar()->addMenu(QObject::tr("  计  算  "));
    QAction *mean= new QAction(QObject::tr("  平  均  "),parent);
    formula->addAction(mean);
    connect(mean,&QAction::triggered,this,&two_electrodes::mean);
    ...
    connect(plot_graphs,&QAction::triggered,this,&two_electrodes::plot_graphs);
...
two_electrodes::~two_electrodes()
{
    delete per1;
    delete k;
    delete ui;
}
void two_electrodes::open_window()
{
...
void two_electrodes::new_row()
{
    int Row_count=ui->tableWidget->rowCount()+1;
    ui->tableWidget->setRowCount(Row_count);
}
void two_electrodes::new_col()
{
...
void two_electrodes::delete_row()
{
    int Row_count=ui->tableWidget->rowCount()-1;
    ui->tableWidget->setRowCount(Row_count);
}
void two_electrodes::delete_col()
{
    int Col_count=ui->tableWidget->columnCount()-3;
    ui->tableWidget->setColumnCount(Col_count);
}
void two_electrodes::clear_data()
{
    ui->tableWidget->clearContents();
}
void two_electrodes::mean()
{

}
void two_electrodes::plot_graphs()
{

}
void two_electrodes::on_comboBox_activated(const QString &arg1)
{
...
void two_electrodes::on_comboBox_2_activated(const QString &arg1)
{
...
void two_electrodes::quit()
{
    serial->Clear_Buffer(serial->Traversing_Data_Pointer,20);
    this->hide();
}
void two_electrodes::closeEvent(QCloseEvent *event)
{
...
void two_electrodes::on_pushButton_clicked()
{
    serial->Send_Command("01COSAMPST\0",11,1,300);
}

void two_electrodes::on_tableWidget_cellDoubleClicked(int row, int column)
{
    k=new keyboard();
...
void two_electrodes::on_pushButton_2_clicked()
{
    serial->Clear_Buffer(serial->Traversing_Data_Pointer,20);
    if(Row_data!=0)
        Col_data+=1;
...
void two_electrodes::receiveData(QString data)
{
    if(!data.isEmpty())
    {
...
void two_electrodes::on_pushButton_3_clicked()
{
    serial->Clear_Buffer(serial->Traversing_Data_Pointer,20);
    if(Row_data!=0)
        Row_data+=1;
...
void two_electrodes::Receive_Data(bool sign)
{
    if(sign)
    {
        QStringList data_1;
...
void two_electrodes::Receive_Data_Filled(bool sign)
{
    if(sign)
    {
        QStringList data_1;
...
void two_electrodes::Fill_Data(QStringList data)
{
    if(!data.isEmpty())
    {
